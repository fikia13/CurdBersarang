...
function TopicList() {
    ...
    const deleteTopic = async(idCourse, idTopic) => {
        try{
            const response = await axios.delete(`http://localhost:5000/topics/${idCourse}/${idTopic}`);
            getTopic();
        }catch(err){
            console.log(err);
        }
                ...
                <tbody>
                {
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button className='button is-small is-danger' onClick={()=> deleteTopic(id ,tp.id)}>
                                    Delete</button>
                            </td>
                        </tr>
                    ))}
